        ...
        Column(Modifier.fillMaxSize().padding(padding).padding(16.dp)){
            ...
            LazyColumn(Modifier.weight(1f),verticalArrangement=Arrangement.spacedBy(10.dp),contentPadding=PaddingValues(vertical=10.dp)){
                items(funnels,key={it.id}){f->
                    val spent=expenses.filter { it.funnelId == f.id && (it.funnelCurrency ?: it.currency).equals(f.currency, true) }.sumOf { it.funnelAmount ?: it.amount }; val otherCurrencyLines=expenses.filter{it.funnelId==f.id}.groupBy{(it.funnelCurrency ?: it.currency).uppercase()}.filterKeys{!it.equals(f.currency,true)}.map{(currency,rows)->currency+" "+"%,.2f".format(rows.sumOf{it.funnelAmount ?: it.amount})}; val remaining=f.limit-spent
